...
- Tạo pipeline và trỏ đến repo github của bạn:
  Trang chủ -> New Item -> Điền tên pineline rồi chọn `pipeline` sau đó ấn `OK`-> Tiếp tục click sang tab `Pipeline` chọn `Pipeline script from SCM` ở mục Definition -> chọn `Git` ở mục SCM -> Điền links repo của bạn vào muc `Repository URL` -> Save
...
> Mình thực hiện CD bằng cách sau khi test và chạy code thành công, mình sẽ build thành một image Docker -> push nó lên registry (Các bước trước này đêu thực hiện trên server chạy Jenkins `192.168.0.105`) -> Chạy service bởi K8s ở Server `192.168.0.106`
...
Truy cập vào `server k8s` sau khi build Jenkinsfile thành công, lấy địa chỉ ip của service mới deploy bằng cách:

```console
minikube service simple-java --url
```

Sau đó gọi API `tests` để xem kết quả
...
- [Cài đặt Jenkins và tạo pipeline CI với Jenkins(nguồn tham khảo chính viết nên bài viết này)](https://www.jenkins.io/doc/tutorials/build-a-java-app-with-maven/)
